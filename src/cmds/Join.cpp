...
void Server::Join(Client &client,std::vector<std::string> params)
{
    ...
    if (InvalidLetter(params[0]) || params[0][0] != '#')
        return  sendServerToClient(client, ERR_UNKNOWNERROR(client._nickname, "JOIN", "Forbidden letter in use as Channel name or didn't use #."));
    if (IsExistChannel(params[0]))
    {
        ...
        else if (params.size() < 2 && ChaKeyCheck(params[0]))
